-- Nips Roll (NRG) - 3.3.5a compatible (Project Epoch)
-- v1.0.2: PLAYER_LOGIN bootstrap + dual slash (/nrg, /nips)
...
-- Early slashes so they exist even if later code errors
SLASH_NRG1  = "/nrg"
SLASH_NRG2  = "/nips"   -- backup slash
SlashCmdList["NRG"] = function()
  if NRG and NRG.Toggle then NRG.Toggle() else
    DEFAULT_CHAT_FRAME:AddMessage("|cFF66C7FF[NRG]|r Loaded. If the window didn’t open, try /reload then /nrg again.")
  end
end
...
local state = {
  phase="IDLE", mode="HILO", channel="SAY",
  betGold=10, rollMin=1, rollMax=100,
...
local function say(chan,msg)
  if not canUseChannel(chan) then sys("Channel "..chan.." not available; showing locally:\n"..msg); return end
...
local function MakeBtn(parent,text,w,h,onClick)
  local b=CreateFrame("Button",nil,parent,"UIPanelButtonTemplate"); b:SetSize(w,h); b:SetText(text); b:SetScript("OnClick",onClick); return b
end
...
local function MakeEdit(parent,w,h,numeric)
  local e=CreateFrame("EditBox",nil,parent,"InputBoxTemplate"); e:SetSize(w,h); e:SetAutoFocus(false); e:SetTextInsets(6,6,2,2)
...
local function UpdateModeUI()
  ...
  if hilo then ui.rollMin:Enable(); ui.rollMax:Enable(); ui.drMax:Disable(); ui.p1Edit:Disable(); ui.p2Edit:Disable()
  else ui.rollMin:Disable(); ui.rollMax:Disable(); ui.drMax:Enable(); ui.p1Edit:Enable(); ui.p2Edit:Enable() end
  NRG.UpdateRoster()
end
...
local function BuildUI()
  ...
  MakeFS(f,"NIPS ROLL (NRG) - Host Panel",14):SetPoint("TOP",0,-12)
  ...
  for _,c in ipairs(chans) do
    local ck=MakeCheck(f,c,function(self) for _,o in ipairs(ui.chanChecks) do o:SetChecked(false) end; self:SetChecked(true); state.channel=c; NRG.UpdateRoster() end)
  ...
  ui.hilo=MakeCheck(f,"Hi/Lo",function() ui.hilo:SetChecked(true); ui.death:SetChecked(false); state.mode="HILO"; UpdateModeUI() end)
  ui.hilo:SetPoint("LEFT",modeLbl,"RIGHT",10,0); ui.hilo:SetChecked(true)
  ui.death=MakeCheck(f,"Death Roll (1v1)",function() ui.hilo:SetChecked(false); ui.death:SetChecked(true); state.mode="DEATH"; UpdateModeUI() end)
  ...
  ui.betEdit:SetScript("OnTextChanged",function(self) local v=tonumber(self:GetText()) or state.betGold; state.betGold=math.max(0,math.floor(v)); NRG.UpdateRoster() end)

  local rollLbl=MakeFS(f,"Roll Range:",12); rollLbl:SetPoint("TOPLEFT",18,-130)
  ui.rollMin=MakeEdit(f,50,20,true); ui.rollMin:SetPoint("LEFT",rollLbl,"RIGHT",10,0); ui.rollMin:SetText(tostring(state.rollMin))
  ui.rollMin:SetScript("OnTextChanged",function(self) local v=tonumber(self:GetText()) or state.rollMin; state.rollMin=math.max(1,math.floor(v)); if state.rollMax<state.rollMin then state.rollMax=state.rollMin end; ui.rollMax:SetText(tostring(state.rollMax)); NRG.UpdateRoster() end)
  MakeFS(f," - ",12):SetPoint("LEFT",ui.rollMin,"RIGHT",5,0)
  ui.rollMax=MakeEdit(f,50,20,true); ui.rollMax:SetPoint("LEFT",ui.rollMin,"RIGHT",25,0); ui.rollMax:SetText(tostring(state.rollMax))
  ui.rollMax:SetScript("OnTextChanged",function(self) local v=tonumber(self:GetText()) or state.rollMax; state.rollMax=math.max(state.rollMin,math.floor(v)); NRG.UpdateRoster() end)

  local drLbl=MakeFS(f,"Death Roll Start Max:",12); drLbl:SetPoint("TOPLEFT",18,-160)
  ui.drMax=MakeEdit(f,80,20,true); ui.drMax:SetPoint("LEFT",drLbl,"RIGHT",10,0); ui.drMax:SetText("1000")
  ui.drMax:SetScript("OnTextChanged",function(self) local v=tonumber(self:GetText()) or 1000; v=math.max(2,math.floor(v)); state.dr.max=v; if state.phase~="DEATHROLL" then state.dr.currentMax=v end; NRG.UpdateRoster() end)
  ...
  ui.p1Edit:SetScript("OnTextChanged",function(self) local v=strtrim(self:GetText()); if v=="" then v=nil end; state.dr.p1=v; NRG.UpdateRoster() end)
  ...
  ui.p2Edit:SetScript("OnTextChanged",function(self) local v=strtrim(self:GetText()); if v=="" then v=nil end; state.dr.p2=v; NRG.UpdateRoster() end)
  ...
  ui.startBtn=MakeBtn(f,"Start Rolling!",120,24,function()
    if state.mode=="HILO" then
      ...
      if not state.dr.p1 or not state.dr.p2 or state.dr.p1==state.dr.p2 then sys("Set P1 and P2 (distinct names) before starting Death Roll."); return end
  ...
  ui.cancelBtn=MakeBtn(f,"Cancel/Reset",120,24,function() announce("Game cancelled by host."); resetGame() end)
  ...
  MakeFS(f,"|cFFAAAAAA/nrg or /nips to show/hide|r",11):SetPoint("BOTTOMRIGHT",-12,10)
...
local function announceHiLoResult()
  ...
  for _,n in ipairs(state.ordered) do local r=state.joiners[n].roll; if r then if r>highV then highV=r; highN=n end; if r<lowV then lowV=r; lowN=n end end end
...
-- Wrath system roll: "Name rolls 42 (1-100)"
local ROLL_PATTERN = "^(.+) rolls (%d+) %((%d+)%-(%d+)%)$"
local function parseRoll(line) local a,b,c,d = line:match(ROLL_PATTERN); if a and b and c and d then return trimRealm(a),tonumber(b),tonumber(c),tonumber(d) end end
...
evt:SetScript("OnEvent", function(_,event,...)
  ...
  elseif event=="CHAT_MSG_SYSTEM" then
    local name,roll,rmin,rmax = parseRoll(...)
    if not name then return end
    if state.mode=="HILO" then handleHiLoRoll(name,roll,rmin,rmax) else handleDeathRoll(name,roll,rmin,rmax) end
    return
  end
  -- joining
  local msg, author = ...
...
-- Register once; PLAYER_LOGIN guarantees UI build even if folder name differs
